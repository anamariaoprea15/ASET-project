...
class Test_RREF(unittest.TestCase):
    ...
    def test_reduced_row_echelon_form(self):
        ...
        for count in range(100):
            print(count)
            random_rows = random.randint(100, 1200)
            random_columns = random.randint(100, 1200)
            
            random_matrix = [[random.randint(1, 100) for _ in range(random_columns)] for _ in range(random_rows)]
            result_matrix = tools.reduced_row_echelon_form(np.array(random_matrix))

            assert self.is_RREF(result_matrix) == 1
